...
def actualpatterns(x):
    patterns = []
    if x >= 128:
        patterns.append(128)
        x = x % 128
        return patterns + actualpatterns(x)
    elif x >= 64:
        patterns.append(64)
        x = x % 64
        return patterns + actualpatterns(x)
...
def generate(rules, source):
    for rule in rules:
        if source == rule:
            return rules[source]
    return '.'

def cellular_automaton(genesource, patterns, generaterates):
    samplesource = genesource[-1] + genesource + genesource[0]
    rules = getrules(actualpatterns(patterns))
    result = ''
    sample = samplesource[:3]
    if generaterates == 1:
        while len(sample) > 2:
            result += generate(rules, sample)
        ...
        while len(sample) > 2:
            ...
            samplesource = samplesource[1:]
            sample = samplesource[:3]
        result = cellular_automaton(result, patterns, generaterates - 1)
    return result
